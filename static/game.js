    ...
    game_data.then(function (data) {
        ...
        for (var i = 0; i < roommate_names.length; i++) {
            if (roommate_names[i] == turn) {
                document.getElementById("turn-circle" + (i + 1)).style.opacity = 1;
            } else {
                document.getElementById("turn-circle" + (i + 1)).style.opacity = 0;
            }
        }
...
// TODO: Make a function to sort by half suit so they look nicer in hand.
...
function organize_by_half_suit(hand) {
    var organized_hand = [];
    for (var half_suit = 0; half_suit < half_suits.length; half_suit++) {
